module webwalrus_graph::graph_metadata {
    ...
    struct GraphMetadata has key, store {
        id: UID,
        name: String,
        description: String,
    ...
    struct GraphUpdated has copy, drop {
        graph_id: ID,
        owner: address,
        name: String,
        blob_id: String,
    ...
    struct GraphDeleted has copy, drop {
        graph_id: ID,
        owner: address,
        name: String,
    }

    struct GraphShared has copy, drop {
        graph_id: ID,
        owner: address,
        shared_with: address,
    }
    ...
    public entry fun update_graph_metadata(
        ...
        // Update tags (simplified - remove old, add new)
        let mut new_tags = vector::empty<String>();
        let mut i = 0;
        while (i < vector::length(&tags)) {
            let tag = vector::borrow(&tags, i);
            vector::push_back(&mut new_tags, string::utf8(*tag));
            i = i + 1;
        };
        graph.tags = new_tags;
    ...
    public fun get_public_graph_ids(registry: &GraphRegistry): vector<ID> {
        ...
        // In a real scenario, you'd need to iterate through the table
        result
    }
